...
const norm = (s: string = ""): string => toHalf(s).trim().toLowerCase().replace(/[\s　]/g, "");
...
// 任意の入力（CSV/JSON/古い保存）→厳密な Card に変換
...
export default function App() {
  ...
  const filtered = useMemo(() => {
    return cards
      ...
      .filter((c) => {
        if (psaFilter === "10") return Number(c.psa) === 10;
        if (psaFilter === "9以下") return Number(c.psa) !== 10;
        return true;
      });
  }, [cards, psaFilter]);
  ...
  function nextQuestion() {
    if (!filtered.length) {
      toast("カードがありません。データをインポートしてください。");
      return;
    }
  ...
  useEffect(() => {
    if (!current && filtered.length) nextQuestion();
    ...
    Papa.parse(file, {
      ...
      complete: (res: Papa.ParseResult<any>) => {
        try {
          ...
          const mapped: Card[] = rows.map(toCard).filter((x) => x.name && Number.isFinite(x.price));
          ...
          toast.error("CSV読み込み失敗: " + e.message);
        }
      },
  ...
  function importJSON(text: string) {
    try {
      ...
      toast.error("JSON読み込み失敗: " + e.message);
    }
  }
  ...
  }, [grade, nextQuestion]); // eslint-disable-line react-hooks/exhaustive-deps
                    ...
                    <div className="grid gap-4">
                      ...
                      <div className="grid gap-3">
                        ...
                        <div className="grid gap-2">
                          ...
                          <Input
                            value={ansPrice}
                            onChange={(e) => setAnsPrice(e.target.value)}
                            inputMode="numeric"
                            placeholder="例：58000"
                            className="h-12 text-lg"
                          />
                        ...
                        <div className="flex gap-3 pt-2">
                          <Button size="lg" className="flex-1" onClick={grade}>
                            <Check className="mr-2 h-4 w-4" />
                            採点（Enter）
                          </Button>
                          <Button size="lg" variant="secondary" className="flex-1" onClick={nextQuestion}>
                        ...
                        {showAnswer && (
                          ...
                          >
                            ...
                            <div className="font-semibold text-lg">
                              {current.name} / {current.psa ? `PSA${current.psa} / ` : ""}
                      ...
                      {summary.last5.map((r, i) => (
                        <div key={i} className="text-sm p-2 rounded-lg border bg-white flex items-center justify-between">
                          <div className="truncate mr-2">
                            {r.correct ? "✅" : "❌"} {r.correctName} / {r.correctPrice.toLocaleString()}円
                          </div>
                          <div className="text-gray-600">
                            → {r.answeredName} / {r.answeredPrice.toLocaleString()}円
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <UICard>
              ...
              <CardContent className="grid md:grid-cols-2 gap-6">
                ...
                <div className="grid gap-4">
                  <div className="grid gap-2">
                    <Label>
                      CSVインポート <span className="text-xs text-gray-500">（推奨ヘッダ：IMG_URL, NAME, PSA, PRICE, ACTIVE, ALIASES）</span>
                    </Label>
                      ...
                      {results
                        ...
                        .map((r, i) => (
                          <tr key={i} className="border-b">
                            ...
                            <td className="p-2 whitespace-nowrap">
                              {r.correct ? "✅ 正解" : "❌"}（名:{r.nameOk ? "○" : "×"} 価:{r.priceOk ? "○" : "×"}）
                            </td>
                          </tr>
                        ))}
                    </tbody>
        ...
        <UICard>
          <CardContent className="text-xs text-gray-500 p-4">
            ...
            <ul className="list-disc ml-5 space-y-1">
              ...
              <li>
                CSV は <b>IMG_URL, NAME, PSA, PRICE, ACTIVE, ALIASES</b> ヘッダ対応
              </li>
            </ul>
          </CardContent>
        </UICard>
